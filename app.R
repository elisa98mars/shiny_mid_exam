...
second_panel <- tabPanel(
    ...
    sidebarLayout( position = "right",
        ...
        sidebarPanel(
          ...
          checkboxInput("member_plot", "By membership")
...
server <- function(input, output) {
    output$customer_plot <- renderPlot({
      ...
      if (input$gender_plot & !input$member_plot) {
        a + geom_histogram(aes(fill= Gender),
                           color= "black",
                           position = "dodge",
                           binwidth = 0.5)
      }
      else if (!input$gender_plot & input$member_plot) {
        a + geom_histogram(aes(fill= MembershipCard),
                           color= "black",
                           position = "dodge",
                           binwidth = 0.5) + 
